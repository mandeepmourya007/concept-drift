...
AUDIO_DATA_PATH = os.path.join(os.getcwd(), 'concept-drift/data')
TFRECORD_PATH = os.path.join(os.getcwd(), 'concept-drift/tfrecords')
    ...
    def load_data(self, normal_index=3):
        ...
        for i, label in enumerate(file_list):
            ...
            for file in glob.glob(os.path.join(AUDIO_DATA_PATH, label, '*.wav')):
                sys.stdout.write(' Extracting {0}...'.format(
                    os.path.split(file)[-1]))

                raw, sr = librosa.load(file, sr=16300, dtype=np.float32)
                # STFT (window_size=2048, overlap_length=512)
                data = abs(librosa.stft(raw, n_fft=2046, hop_length=512))

                while True:  # segmentation
                    try:
                        segment = np.reshape(data[:, :self.data_width],
                                             [self.data_height, self.data_width, self.data_channel])
                    except:
                        break
                    audio.append((segment-np.mean(segment))/np.std(segment))
                    # monitoring overlap ratio = 1/3
                    data = data[:, self.data_width//3:]

                print(' {0}'.format(len(audio)))
                print(
                    ' -- Memory Usage: {0:>0.3f}'.format(process.memory_percent()))
        ...
        self.test = Sub_Dataset(np.asarray(test_audio),
                                np.asarray(test_labels))

        print(' Total Amount: Train({0}), Test({1})'.format(
            len(self.train.data), len(self.test.data)))
...
class Sub_Dataset():
    ...
    def next_batch(self, batch_size):
        ...
        if(next_index < len(self.data)):
            batch_data, batch_label = self.data[self.batch_index:
                                                next_index], self.labels[self.batch_index:next_index]
...
class TFRecord_Saver():
    ...
    def save_tfrecord(self, dataset, tfrec_name):
        ...
        save_path = os.path.join(
            self.save_path, tfrec_name+'_set_%02d.tfrecords.gz' % file_num)

        while os.path.exists(save_path):
            file_num += 1
            save_path = os.path.join(
                TFRECORD_PATH, tfrec_name+'_set_%02d.tfrecords.gz' % file_num)

        print(' Saving Dataset...({0}_set_{1:02})'.format(
            tfrec_name, file_num))
        std_time = time.time()
        writer = tf.python_io.TFRecordWriter(
            save_path, tf.python_io.TFRecordOptions(2))
...
class TFRecord_Reader():
    def __init__(self, values, tfrec_name, batch_size, num_epochs=None, shuffle=None):
        ...
        if num_epochs == 1:
            self.allow_smaller_final_batch = True
        else:
            self.allow_smaller_final_batch = False

        filenames = [file for file in glob.glob(
            os.path.join(TFRECORD_PATH, tfrec_name+'*.tfrecords.gz'))]
        ...
        if shuffle:
            self.mini_batch = self.next_shuffle_batch(batch_size)
        else:
            self.mini_batch = self.next_batch(batch_size)
    ...
    def read_tfrecord(self):
        ...
        example = tf.parse_single_example(
            serialized_example, features=features)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        if len(os.listdir(TFRECORD_PATH)) < 1:
            ...
            tfrecord.save_tfrecord(dataset=dataset.train,
                                   tfrec_name='training')
        ...
        X = tf.placeholder(tf.float32, shape=[
                           None]+[dataset.data_height, dataset.data_width, dataset.data_channel])
        ...
        coord.join(threads)
        sess.close()
print("""  labels for other dataset
TV Series 		         
 Reception Area 		
 Outdoor Canteen-1
 Lab-1 		
