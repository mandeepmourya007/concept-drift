...
parser.add_argument('--train', action='store', type=str2bool,
                    dest='training', default=False)
config, unparsed = parser.parse_known_args()

# to ignore tensorflow SSE instruction warning
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

model = detect_drift(trainable=config.training)
...
def training_model():
    training_steps = 3  # 10000
    training_batch_size = 32

    training_loss = tf.get_variable(
        name='Training-loss', shape=[], initializer=tf.zeros_initializer())
    avg_normal_loss = tf.get_variable(
        name='Normal-loss', shape=[], initializer=tf.zeros_initializer())
    avg_abnormal_loss = tf.get_variable(
        name='Abnormal-loss', shape=[], initializer=tf.zeros_initializer())
    ...
    for step in range(training_steps):
        ...
        sess.run(model.train, feed_dict={
                 model.X: X_data, model.Y: Y_data, model.is_training: True})
        sys.stdout.write(" Step: %4d ...\r" % step)

        if step % 100 == 0:
            normal_loss, abnormal_loss = [], []
            loss_curr = sess.run(model.loss, feed_dict={
                                 model.X: X_data, model.Y: Y_data, model.is_training: True})

            for i in range(64):  # test set length: 2,113 / test batch size: 33
                X_data, Y_data = sess.run(dataset.test.mini_batch)
                # normal index=3
                Normal_X, Abnormal_X = X_data[Y_data.T[3]
                                              == 1], X_data[Y_data.T[3] != 1]
                if len(Normal_X) > 0:
                    normal_loss.append(sess.run(model.loss, feed_dict={
                                       model.X: Normal_X, model.is_training: False}))
                if len(Abnormal_X) > 0:
                    abnormal_loss.append(sess.run(model.loss, feed_dict={
                                         model.X: Abnormal_X, model.is_training: False}))
...
def validation():
    ...
    while not coord.should_stop():
        ...
        score = sess.run(model.loss, feed_dict={
                         model.X: X_data, model.Y: Y_data, model.is_training: False})

        if label == 3:
            normal_score.append(score)
        else:
            abnormal_score[label].append(score)
    ...
    for i in range(3):
        while len(abnormal_score[i]) > 0:
            avg_abnormal[i].append(
                np.mean(abnormal_score[i][:seg_num_for_ensemble]))
    ...
    label_list = ['B-line', 'C-line with intermittent noise',
                  'Non-greased C-line']
    for i in range(3):
        false_pos_rate, true_pos_rate, _ = sklearn.metrics.roc_curve(
            np.concatenate((np.zeros(len(avg_normal)),
                            np.ones(len(avg_abnormal[i]))), axis=0),
